    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> DeleteUser(int userId)
        {
            ...
            try
            {
                ...
                var originalFullName = user.FullName ?? "Unknown";
                
                // Only update the Status field - use direct SQL update to avoid entity tracking issues
                await _context.Database.ExecuteSqlRawAsync(
                    "UPDATE Users SET Status = {0} WHERE UserId = {1}",
                    "Deleted", userId);
                
                await LogAuditActionAsync("Delete", "User", userId, $"Deleted user: {originalFullName}");
        ...
        public async Task<IActionResult> SuspendUser(int userId)
        {
            ...
            try
            {
                ...
                await LogAuditActionAsync("Suspend", "User", userId, $"Suspended user: {originalFullName}");
            ...
            catch (DbUpdateException dbEx)
            {
                // Get inner exception for more details
                var innerException = dbEx.InnerException?.Message ?? dbEx.Message;
                var stackTrace = dbEx.InnerException?.StackTrace ?? "";
                return Json(new { success = false, message = $"Database error: {innerException}", details = stackTrace });
            }
            catch (Exception ex)
            {
                // Include inner exception details if available
        ...
        // Made safe with try-catch to prevent CRUD operations from failing if AuditLogs table doesn't exist
        private async Task LogAuditActionAsync(string action, string entityType, int? entityId = null, string? description = null)
        {
            ...
            {
                // Silently fail audit logging - don't break CRUD operations
                // Log to console in development if needed
                // In production, you might want to log this to a file or external logging service
        ...
        public async Task<IActionResult> DeactivateUser(int userId)
        {
            ...
            try
            {
                ...
                await LogAuditActionAsync("Deactivate", "User", userId, $"Deactivated user: {originalFullName}");
        ...
        // GET: /Admin/GetSubscribers
        [HttpGet]
        public async Task<IActionResult> GetSubscribers()
        {
            try
            {
                var subscribers = await (from ap in _context.AuthorPlans
                                        join u in _context.Users on ap.UserId equals u.UserId
                                        join p in _context.Plans on ap.PlanId equals p.PlanId
                                        orderby ap.CreatedAt descending
                                        select new
                                        {
                                            ...
                                            endDate = ap.EndDate.ToString("yyyy-MM-dd"),
                                            isActive = ap.IsActive == 1,
                                            status = ap.IsActive == 1 ? (ap.EndDate > DateTime.UtcNow ? "Active" : "Expired") : "Cancelled",
                                            paymentReference = ap.PaymentReference ?? "",
                                            createdAt = ap.CreatedAt.ToString("yyyy-MM-dd HH:mm:ss")
                                        }).ToListAsync();

                return Json(new { success = true, subscribers });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public async Task<IActionResult> GetPayments()
        {
            try
            {
                var payments = await (from ab in _context.AuthorBills
                                     join u in _context.Users on ab.UserId equals u.UserId into userGroup
                                     from user in userGroup.DefaultIfEmpty()
                                     orderby ab.CreatedAt descending
                                     select new
                                     {
                                         transactionId = ab.BillId,
                                         userId = ab.UserId,
                                         userName = user != null ? user.FullName : "Unknown",
                                         userEmail = ab.UserEmail ?? (user != null ? user.UserEmail : ""),
                                         amount = ab.TotalAmount,
                                         currency = ab.Currency ?? "usd",
                                         status = ab.Status ?? (ab.IsActive == 1 ? "Paid" : "Pending"),
                                         paymentReference = ab.PaymentReference ?? "",
                                         description = ab.Description,
                                         createdAt = ab.CreatedAt.ToString("yyyy-MM-dd HH:mm:ss"),
        ...
        // GET: /Admin/GetSubscriptionStats
        [HttpGet]
        public async Task<IActionResult> GetSubscriptionStats()
        {
            try
            {
                var totalSubscribers = await _context.AuthorPlans.CountAsync();
                var activeSubscribers = await _context.AuthorPlans.CountAsync(ap => ap.IsActive == 1 && ap.EndDate > DateTime.UtcNow);
                var expiringSoon = await _context.AuthorPlans.CountAsync(ap => ap.IsActive == 1 && ap.EndDate > DateTime.UtcNow && ap.EndDate <= DateTime.UtcNow.AddDays(30));

                var monthlyRevenue = await (from ab in _context.AuthorBills
                                           where ab.CreatedAt >= DateTime.UtcNow.AddMonths(-1) && ab.Status == "Paid"
                                           select ab.TotalAmount).SumAsync();

                var totalRevenue = await (from ab in _context.AuthorBills
                                         where ab.Status == "Paid"
                                         select ab.TotalAmount).SumAsync();

                var activePlans = await _context.AuthorPlans
                    .Where(ap => ap.IsActive == 1 && ap.EndDate > DateTime.UtcNow)
                    .CountAsync();

                var retentionRate = totalSubscribers > 0 ? (double)activePlans / totalSubscribers * 100 : 0;
                ...
                return Json(new
                {
                    ...
                    stats = new
                    {
                        ...
                        activeSubscribers,
                        expiringSoon,
                        monthlyRevenue = Math.Round((decimal)monthlyRevenue, 2),
                        totalRevenue = Math.Round((decimal)totalRevenue, 2),
        ...
        // ========== USER PREFERENCES (REMOVED - Now uses localStorage) ==========
        // These endpoints are kept for backward compatibility but return success without database operations
        // Preferences are now stored in browser localStorage
        
        // POST: /Admin/SaveUserPreference (Deprecated - uses localStorage now)
        [HttpPost]
        public IActionResult SaveUserPreference(string key, string value)
        {
            // Preferences now stored in localStorage (client-side)
            return Json(new { success = true, message = "Preference saved to browser (localStorage)" });
        }

        // GET: /Admin/GetUserPreference (Deprecated - uses localStorage now)
        [HttpGet]
        public IActionResult GetUserPreference(string key)
        {
            // Preferences now loaded from localStorage (client-side)
