...
namespace EBookDashboard.Controllers
{
    public class PaymentsController : Controller
    {
        ...
        public async Task<IActionResult> CheckoutPayment(int? billId = null, string featureIds = null)
        {
            ...
            // Use consistent user identification - try Email first, then NameIdentifier, then Name
            ...
            {
                user = await _context.Users.FirstOrDefaultAsync(u => u.UserEmail == userEmail);
            }
            
            // If not found by email, try by NameIdentifier (userId)
            if (user == null && !string.IsNullOrEmpty(userIdClaim) && int.TryParse(userIdClaim, out int userIdFromClaim))
            {
                user = await _context.Users.FirstOrDefaultAsync(u => u.UserId == userIdFromClaim);
            }
            
            // Fallback to FullName if still not found
            if (user == null && !string.IsNullOrEmpty(username))
            {
                user = await _context.Users.FirstOrDefaultAsync(u => u.FullName == username);
            }
            ...
            // Verify ownership - check both UserId and UserEmail for extra security
            if (authorBill.UserId != userId || (authorBill.UserEmail != null && authorBill.UserEmail != user.UserEmail))
            {
        ...
        public async Task<IActionResult> PaymentSummary(int? billId = null)
        {
            ...
            if (user == null) return Unauthorized("User not found. Please log in again.");
            ...
            // Verify ownership - check both UserId and UserEmail for extra security
            if (bill.UserId != user.UserId || (bill.UserEmail != null && bill.UserEmail != user.UserEmail))
            {
                return Unauthorized("This bill does not belong to you");
            }
        ...
        public async Task<IActionResult> CreateCheckoutSession()
        {
            try
            {
                ...
                if (user == null) return Unauthorized(new { success = false, message = "User not found. Please log in again." });
        ...
        public async Task<IActionResult> Checkout1(int planId = 0, int? billId = null)
        {
            ...
            if (billId.HasValue && billId.Value > 0)
            {
                ...
                if (authorBill.UserId != userId || authorBill.AuthorId != authorId || 
                    (authorBill.UserEmail != null && authorBill.UserEmail != user.UserEmail))
                {
                    return Unauthorized("This bill does not belong to you");
                }
