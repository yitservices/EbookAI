    ...
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> Profile(PlanFeatures f)
        {
            try
            {
                ...
                if (activePlan != null)
                {
                    // Get features for this plan - handle potential NULL values
            ...
            catch (InvalidCastException ex) when (ex.Message.Contains("DBNull") && ex.Message.Contains("Boolean"))
            {
                // Handle DBNull to Boolean casting error
                // This can happen if boolean fields in the database have NULL values
                // Log the error and return a default view
                System.Diagnostics.Debug.WriteLine($"DBNull to Boolean cast error in Profile: {ex.Message}");
                ...
                // Return a simplified view model
                var errorViewModel = new DashboardProfileViewModel
                {
                    ...
                    BillingCycle = "Monthly",
                    PaymentMethod = "**** 4242",
                    PlanFeatures = new List<PlanFeatureViewModel>()
                };
        ...
        public async Task<IActionResult> Styling(int? bookId = null)
        {
            ...
            // Check if user has styling feature via AuthorPlans -> AuthorPlanFeatures -> Features
            // Join via BillId (if both have BillId) or AuthorId + UserId combination
            var hasStylingFeature = await (from ap in _context.AuthorPlans
                                          join apf in _context.AuthorPlanFeaturesSet on 
                                              new { ap.AuthorId, ap.UserId } equals new { apf.AuthorId, apf.UserId } into planFeatures
                                          from pf in planFeatures.DefaultIfEmpty()
                                          join f in _context.Features on (pf != null && pf.FeatureId != null ? pf.FeatureId.Value : 0) equals f.Id into features
                                          from feature in features.DefaultIfEmpty()
                                          where ap.UserId == user.UserId 
                                                && ap.IsActive == 1 
                                                && ap.EndDate > DateTime.UtcNow
                                                && feature != null
                                                && (feature.Name.ToLower().Contains("style") ||
                                                    feature.Key.ToLower().Contains("style"))
                                          select feature).AnyAsync();
        ...
        public async Task<IActionResult> Subscriptions()
        {
            ...
            // Load user's purchased features via AuthorPlans -> AuthorPlanFeatures -> Features
            // Join via AuthorId + UserId combination (both tables have these fields)
            var userFeatures = await (from ap in _context.AuthorPlans
                                     join apf in _context.AuthorPlanFeaturesSet on 
                                         new { ap.AuthorId, ap.UserId } equals new { apf.AuthorId, apf.UserId }
                                     join f in _context.Features on (apf.FeatureId != null ? apf.FeatureId.Value : 0) equals f.Id
                                     where ap.UserId == user.UserId 
                                           && ap.IsActive == 1 
                                           && ap.EndDate > DateTime.UtcNow
                                     select new
        ...
        public async Task<IActionResult> Settings()
        {
            ...
            // Notification preferences now use localStorage (client-side)
            // Default to enabled
            ViewBag.NotificationsEnabled = true;
            ViewBag.User = user;
            
            return View();
        }

        // Save notification settings - Now uses localStorage (client-side only)
        // This endpoint is kept for backward compatibility but does nothing
        [HttpPost]
        [Route("SaveNotificationSettings")]
        public IActionResult SaveNotificationSettings([FromBody] bool enabled)
        {
            // Notification settings are now stored in localStorage on client-side
            // This endpoint returns success for backward compatibility
            return Json(new { success = true, message = "Settings saved to browser (localStorage)" });
        }
